...
# installs npm dependencies, and ensures the models/ placeholder directories
# exist. Safe to re-run (idempotent) — does not overwrite any existing
# source files. Performs network access (npm install, and npm create vite
# only in the fallback branch below) — run it yourself; it is never executed
# automatically by any agent or tooling in this repo.
#
# Usage:
#   bash scripts/create-project.sh
#
# This script intentionally does NOT download model weight files — see
# models/README.md for that (separate, deliberate step with its own license
...
# --- 4. Ensure models/ placeholder directories exist ------------------------

log "Ensuring models/ placeholder directories exist"
for dir in models/detector models/embedder models/antispoof; do
  mkdir -p "$dir"
...
missing_models=0
[ -f models/detector/scrfd_tiny.onnx ]      || { echo "  missing: models/detector/scrfd_tiny.onnx";      missing_models=1; }
[ -f models/embedder/mobilefacenet.onnx ]   || { echo "  missing: models/embedder/mobilefacenet.onnx";    missing_models=1; }
[ -f models/antispoof/antispoof_tiny.onnx ] || { echo "  missing: models/antispoof/antispoof_tiny.onnx";  missing_models=1; }
...
if [ "$missing_models" -eq 1 ]; then
  ...
  echo "  source/convert real model files. See models/README.md for instructions."
else
...
log "Setup complete"
echo "  Next steps:"
echo "    1. If you saw 'missing model files' above, follow models/README.md"
echo "    2. npm run dev"
