...
- **Current mechanism (the gate): active head-motion challenge.** Implemented in [src/core/LivenessChallenge.ts](src/core/LivenessChallenge.ts) (pure state machine) + [src/components/ChallengeGate.tsx](src/components/ChallengeGate.tsx) (camera + per-frame driver). The user is asked to turn their head; the challenge verifies, from the 5 SCRFD landmarks we already get per frame, that head yaw reached a strong turn in **both** signs after a centered baseline, within a time budget. A static held-up photo cannot produce that. On pass, a single **frontal** aligned face is captured for the embedding.
  - **Mirror-agnostic by design.** We do NOT assume "left = negative yaw" — webcam mirroring and SCRFD's eye-point order are both unknown without per-device calibration, and guessing the sign is exactly the bug class that repeatedly broke the passive model below. Requiring a strong turn in *both* signs needs no sign knowledge; the user just turns both ways.
  - **Deterministic + unit-tested.** Because the logic is a pure `(landmarks, timestamp) → status` function, it is covered by deterministic unit tests ([src/core/LivenessChallenge.test.ts](src/core/LivenessChallenge.test.ts)) feeding synthetic landmark sequences — the first liveness component in this system that can be tested without a camera.
  - **Config:** `liveness.challenge` in [src/core/config.ts](src/core/config.ts) — `centerYawDeg`, `turnYawDeg`, `totalTimeoutMs`, `enabled`. Thresholds are starting points; tune `turnYawDeg`/`centerYawDeg` against a real camera (the on-screen both-sides progress makes this easy to eyeball).
- **Passive CNN component — sourced, then RETIRED from the gate (kept dormant).** A tiny real-vs-spoof CNN (MiniFASNetV2) was sourced and wired, but real-camera testing showed it produced near-identical output (~0.994 on logit index 2) for **both a real live face and a static photo** — it did not discriminate at all (cause undetermined: color order, crop geometry, missing ensemble partner, or conversion output-reorder). Rather than keep chasing it, liveness was pivoted to the active challenge above. The model file, [src/core/LivenessModel.ts](src/core/LivenessModel.ts), and [src/workers/antispoof.worker.ts](src/workers/antispoof.worker.ts) are retained intact but the worker is **no longer spawned**; it can return as an *advisory* (never-blocking) layer if/when its preprocessing is sorted out. The model-free texture/moiré heuristic (`textureHeuristic`) lives in `LivenessModel.ts` alongside it, also dormant.
- **Explicit limitation to document everywhere:** software-only liveness on commodity RGB webcams cannot reliably defeat sophisticated spoofing (high-quality masks, deepfake/video injection, or a recorded video of the subject performing the head turn). The active challenge defeats casual *static-photo* spoofing, which is the realistic threat for a consensual kiosk; it is a basic-assurance layer, not strong security. Do not market or rely on it as more than that.
