...
| [src/components/LivenessPrompt.tsx](src/components/LivenessPrompt.tsx) | Done | "Checking…" indicator + failure messaging (does NOT track blinks — see its docblock) |
| [src/core/LivenessModel.ts](src/core/LivenessModel.ts) | **Works, but model not discriminating — see §2** | Runs the anti-spoof model + texture heuristic. Currently advisory only (`config.liveness.enforce=false`) because the model can't tell live from spoof yet |
...
- [ ] **Real enrollment → match round trip** with an actual human face — not yet explicitly confirmed end-to-end (was blocked by liveness until liveness was made advisory; the enroll→match path itself should now be reachable and is the next thing to confirm).
- [ ] **⚠️ ANTI-SPOOF MODEL DOES NOT WORK YET (the big open item).** Real-camera testing on 2026-06-25 showed the MiniFASNetV2 model outputs a near-identical distribution — `~[0.000, 0.006, 0.994]` (99.4% on logit index 2) — for **both a real live face AND a static photo**. It is not discriminating live from spoof at all. Because of this, `config.liveness.enforce` is set to `false` (advisory mode: score computed + shown, never blocks) so the core enroll/match feature is usable. **Re-approaching this is scheduled for 2026-06-27 (Saturday).** Leading hypotheses to test, in order: (1) **color order** — both the TS code and the original Python validation feed BGR; if the garciafido ONNX export actually expects RGB, both were "wrong together," which would explain the identical-output-for-everything symptom. Try `colorOrder: "RGB"` in the antispoof manifest entry. (2) **crop geometry** — minivision's real crop is `box_w*2.7 × box_h*2.7` (a rectangle proportional to the box) resized to 80×80, NOT the `max(w,h)*2.7` square `cropWithMargin()` currently produces. (3) **ensemble** — the real minivision system sums TWO models (2.7_80x80_MiniFASNetV2 + 4_0_0_80x80_MiniFASNetV1SE) and argmaxes; a single model is known-unreliable. (4) **output order** — the conversion may have reordered classes. **Validation requires testing against BOTH a real face and a real spoof (photo/screen) — a face-only or photo-only test cannot confirm discrimination (that's exactly the trap that hid this until now).** The temporary on-screen + console `[debug]` readout (rawProbs / modelScore / texture / avgVar) is in place to support this; remove it once resolved.
- [ ] **`config.ts` threshold tuning** (`matchThreshold: 0.62`, `liveness.minScore: 0.5`) against real accuracy data — see [privacy-and-testing.md](privacy-and-testing.md) §3.1. Still unvalidated placeholders.
